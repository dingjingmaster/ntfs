...
enum
{
    ...
    NTFS_MNT_FORENSIC               = 0x04000000, /* No modification during mount. */
...
struct _ntfs_volume
{
    ...
    LCN mft_lcn;                                /* Logical cluster number of the data attribute for FILE_MFT. */
    ntfs_inode *mft_ni;                         /* ntfs_inode structure for FILE_MFT. */
    ntfs_attr *mft_na;                          /* ntfs_attr structure for the data attribute of FILE_MFT. */
    ntfs_attr *mftbmp_na;                       /* ntfs_attr structure for the bitmap attribute of FILE_MFT.
                                                 *  Each bit represents an mft record in the $DATA attribute, bit 0 representing mft record 0 and so on.
                                                 *  A set bit means that the mft record is in use and vice versa. */
    ...
    LCN mftmirr_lcn;                            /* Logical cluster number of the data attribute for FILE_MFTMirr. */
    ...
    s32 attrdef_len;                            /* Size of the attribute definition table in bytes. */

    s64 free_clusters;                          /* Track the number of free clusters which greatly improves statfs() performance */
    s64 free_mft_records;                       /* Same for free mft records (see above) */
    BOOL efs_raw;                               /* volume is mounted for raw access to efs-encrypted files */
