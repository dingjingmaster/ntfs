...
static int                      g_mft_size              = 0;        // g_buf 变量长度，默认 27KB。
...
/**
 * @brief 用于计算 64 位 CRC(循环冗余校验)值。
 *  CRC 是一种广泛使用的数据完整性检查机制，用于检测在数据传输或存储中可能发生的错误。
 *
 *  在NTFS中主要：
 *      1. 计算卷引导扇区CRC值，以验证其完整性
 *      2. 计算Master File Table(MFT)记录的CRC值以验证其完整性
 *      3. 计算 NTFS 元数据结构的 CRC 值以检测数据损坏。
 *
 *  CRC工作原理：
 *      1. 将输入数据分割成固定大小的数据块
 *      2. 对每个数据块应用一个预定义的多项式(CRC多项式)进行运算，得到一个CRC校验码
 *      3. 将CRC校验码附加到原始数据上，一起传输或存储
 *      4. 在接收或读取数据时，再次计算数据的CRC校验码并与附加的CRC校验码进行比较，如果一致则数据完整，否则数据发生了错误
 *
 *  CRC计算过程：
 *      1. 初始化CRC寄存器
 *      2. 处理输入数据：
 *          2.1 将输入数据按位逐个移入CRC寄存器
 *          2.2 对于每个输入位
 *              2.2.1 如果当前CRC寄存器最高位为1，则执行异或运算
 *              2.2.2 否则只需将CRC多项式左移一位
 *          2.3 异或运算使用预定义的CRC多项式进行
 *      3. 生成最终CRC校验码
 *          经过上述处理后，CRC寄存器中保存的就是最终的CRC校验码
 */
...
static int insert_non_resident_attr_in_mft_record(MFT_RECORD *m, const ATTR_TYPES type, const char *name, u32 name_len, const IGNORE_CASE_BOOL ic, const ATTR_FLAGS flags, const u8 *val, const s64 val_len, WRITE_TYPE write_type)
{
    ...
    */
    ...
    if (val_len) {
        rl = allocate_scattered_clusters((val_len + g_vol->cluster_size - 1) / g_vol->cluster_size);
        if (!rl) {
            err = -errno;
    ...
    memset(a->reserved1, 0, sizeof(a->reserved1));
    /* FIXME: Allocated size depends on compression. */
    a->allocated_size = cpu_to_sle64((val_len + (g_vol->cluster_size - 1)) & ~(g_vol->cluster_size - 1));
    a->data_size = cpu_to_sle64(val_len);
    ...
    if (flags & ATTR_COMPRESSION_MASK) {
        ...
        a->compression_unit = 0;
        bw = ntfs_rlwrite(g_vol->dev, rl, val, val_len, NULL, write_type);
        if (bw != val_len) {
            ntfs_log_error("Error writing non-resident attribute value.\n");
            return -errno;
        }
...
static BOOL mkntfs_override_vol_params(ntfs_volume *vol)
{
    ...
    if (vol->cluster_size > 4096) {
        if (opts.enable_compression) {
            if (!opts.force) {
                ntfs_log_error("Windows cannot use compression when the cluster size is larger than 4096 bytes.\n");
                return FALSE;
            }
...
static BOOL mkntfs_create_root_structures(void)
{
    ...
    /* FIXME: This should be IGNORE_CASE */
    if (!err) {
        err = add_attr_index_root(m, "$I30", 4, CASE_SENSITIVE, AT_FILE_NAME, COLLATION_FILE_NAME, g_vol->indx_record_size);
    }

    /* FIXME: This should be IGNORE_CASE */
    if (!err) {
        err = upgrade_to_large_index(m, "$I30", 4, CASE_SENSITIVE, &g_index_block);
    }
    ...
    if (!err) {
        ...
        a = ctx->attr;
        err = insert_file_link_in_dir_index(g_index_block, root_ref, (FILE_NAME_ATTR*)((char*)a + le16_to_cpu(a->value_offset)), le32_to_cpu(a->value_length));
        ntfs_attr_put_search_ctx(ctx);
    }
    if (err) {
...
static int mkntfs_redirect(struct mkntfs_options *opts2)
{
    ...
    upcase_crc = crc64(0, (byte*) g_vol->upcase, g_vol->upcase_len * sizeof(ntfschar));
    ...
    if (a->non_resident) {
        ...
        lw = ntfs_rlwrite(g_vol->dev, rl, (const u8*)NULL, g_lcn_bitmap_byte_size, NULL, WRITE_BITMAP);
        err = errno;
        free(rl);
